...
export default function Registro() {
  ...
  const manejarRegistro = async (e) => {
    ...
    try {
      // Forzamos los valores a booleanos dependiendo del select
      const datosRegistro = {
    ...
    <div style={estilos.contenedorPrincipal}>
      <div style={estilos.tarjetaGlass}>
        <div style={estilos.ladoIzquierdo}>
          <h1 style={{ fontSize: '3rem', marginBottom: '1rem' }}>Es hora de viajar</h1>
          <div style={{ padding: '1rem', backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: '15px', display: 'inline-block' }}>
            ...
            <br />
            <Link to="/login" style={{ color: 'white', fontWeight: 'bold', fontSize: '1.2rem', textDecoration: 'none' }}>Sign In ➔</Link>
          </div>
        </div>

        <div style={estilos.ladoDerecho}>
          <h2 style={{ fontSize: '2.5rem', marginBottom: '1.5rem', fontWeight: 'normal' }}>sign up</h2>
          ...
          <form onSubmit={manejarRegistro} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            <div>
              <label style={estilos.label}>Full Name</label>
              <input type="text" name="nombre" placeholder="Your Name Here" onChange={manejarCambio} required style={estilos.input} />
            </div>
            <div>
              <label style={estilos.label}>E-mail</label>
              <input type="email" name="email" placeholder="E-mail Address" onChange={manejarCambio} required style={estilos.input} />
            </div>
            <div>
              <label style={estilos.label}>Account Type (Role)</label>
              {/* AQUÍ ESTÁ EL FIX: Mantenemos el value="Host" pero el texto en español */}
            ...
            <button type="submit" disabled={cargando} style={estilos.boton}>
              {cargando ? 'Creating...' : 'Create Account'}
            </button>
          </form>
        </div>
      </div>
